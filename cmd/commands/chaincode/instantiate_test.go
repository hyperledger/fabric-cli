...
var _ = Describe("ChaincodeInstantiateCommand", func() {
	var (
		cmd      *cobra.Command
	...
	JustBeforeEach(func() {
		cmd = chaincode.NewChaincodeInstantiateCommand(settings)
	})
	...
	It("should create a chaincode instantiate commmand", func() {
		Expect(cmd.Name()).To(Equal("instantiate"))
	...
	It("should provide a help prompt", func() {
		...
		Expect(fmt.Sprint(out)).To(ContainSubstring("instantiate <chaincode-name> <version> <path>"))
	})
})

var _ = Describe("ChaincodeInstantiateImplementation", func() {
	var (
		impl     *chaincode.InstantiateCommand
		err      error
		out      *bytes.Buffer
		settings *environment.Settings
	...
	Describe("Run", func() {
		...
		Context("when resmgmt client succeeds", func() {
			...
			It("should succeed with chaincode instantiate", func() {
				Expect(err).To(BeNil())
				Expect(fmt.Sprint(out)).To(Equal("successfully instantiated chaincode 'mycc'\n"))
			})
		})
		...
		Context("when resmgmt client fails", func() {
			BeforeEach(func() {
				...
				client.InstantiateCCReturns(resmgmt.InstantiateCCResponse{}, errors.New("instantiate error"))
			})

			It("should fail to instantiate chaincode", func() {
				Expect(err).NotTo(BeNil())
				Expect(err.Error()).To(ContainSubstring("instantiate error"))
			})
		})
	})
})
